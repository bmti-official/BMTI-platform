...
# Each block has the type on the first line, and the text on the second.
# Wait, the prompt has multiple paragraphs. Let's parse it differently.
blocks = raw_text.strip().split('\n\n\n') # wait, the format is type \n text
# actually, let's use a simpler parser
current_code = None
current_text = []
...
if not match:
    print("Could not find BODY_GUIDE_DATA")
    exit(1)

dict_content = match.group(1)

# evaluate it roughly to update it
import ast
# We need to parse the object. It's a JS object, let's just do simple regex replacements.
for code, desc in new_items.items():
    escaped_desc = desc.replace('"', '\\"').replace('\n', '\\n')
    # find the line for this code and replace it
    pattern = r"'" + code + r"': \".*?\","
