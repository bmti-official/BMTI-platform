...
// 📊 통계·지표 — 회원 구성, 다이어리 기록(지금 바로 되는 것), 행동 기록(app_events)을 한 장에서 본다.
const GOLD = '#9C6F26';
const ACCENT = '#6B5BB5';
...
export default function MetricsView() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      const u = await supabase.from('users').select('id,bmti_type,created_at,nickname,kakao_gender,kakao_age,app_notification');
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
      ...
      {/* 회원이 어떻게 이뤄져 있나 — 예전 '통계' 탭에 있던 것 */}
      <div>
        <div style={{ fontSize: 14, fontWeight: 900, color: INK, marginBottom: 8 }}>
          회원 구성 <span style={{ fontWeight: 600, color: SUB, fontSize: 12 }}>· 가입한 사람 기준</span>
        </div>
        <StatsView users={users} loading={loading} />
      </div>
